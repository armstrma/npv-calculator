            ...
            <BarChart data={tornadoRippleData} layout="vertical" margin={{ top: 5, right: 18, left: 40, bottom: 5 }} barGap={-20}>
              ...
              <Legend
                payload={showSensitivity
                  ? [
                      { value: '±5% (bold)', type: 'square', color: '#ef4444' },
                      { value: '±10% (medium)', type: 'square', color: '#f87171' },
                      { value: '±20% (subtle)', type: 'square', color: '#fca5a5' },
                    ]
                  : [
                      { value: '-10% / +10% only (default)', type: 'square', color: '#f87171' },
                    ]}
              />

              {showSensitivity && <Bar dataKey="down20" name="-20%" fill="#fca5a5" barSize={20} />}
              <Bar dataKey="down10" name="-10%" fill="#f87171" barSize={showSensitivity ? 14 : 20} />
              {showSensitivity && <Bar dataKey="down5" name="-5%" fill="#dc2626" barSize={8} />}
              {showSensitivity && <Bar dataKey="up20" name="+20%" fill="#bbf7d0" barSize={20} />}
              <Bar dataKey="up10" name="+10%" fill="#4ade80" barSize={showSensitivity ? 14 : 20} />
              {showSensitivity && <Bar dataKey="up5" name="+5%" fill="#16a34a" barSize={8} />}
